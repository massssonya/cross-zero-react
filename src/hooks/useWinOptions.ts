...
export const useWinOptions = ({ items, col, numValueInRow, numMarkers }: IWinOptions) => {

    const getHorizontalWinOptions = useCallback(function (field: ISquareItem[]) {
        let options: TOption[] = [];
        let iteration = 1;
        ...
        return options;
    }, [items, col, numValueInRow, numMarkers]) 

    const getVerticalWinOptions = useCallback(function(field: ISquareItem[]) {
        let options: TOption[] = [];
        for (let i = 0; i < col; i++) {
            let numOptions = 0;
            while (numOptions < numValueInRow) {
    ...
    const getDiagonalWinOptions = useCallback(function(field: ISquareItem[]) {
        const diagonals = [
            { step: col + 1, init: 0 },
